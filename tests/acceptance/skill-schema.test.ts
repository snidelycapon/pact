 ...
 * Scenarios verify:
 *   - garp_request warns when context_bundle is missing required fields defined in schema.json
 *   - garp_request does not warn when all required fields are present
 *   - garp_request skips validation entirely when no schema.json exists
...
/** Add schema.json to the sanity-check skill directory and push. */
...
describe("schema.json: typed skill contracts and validation warnings", () => {
  ...
  it("warns on missing required fields and still submits the request successfully", async () => {
    ...
    await given("the sanity-check skill has a schema.json with 6 required context fields", () => {
  ...
  it("does not include validation_warnings when all required fields are present", async () => {
    ...
    await given("the sanity-check skill has a schema.json", () => {
      seedSchemaJson(ctx.aliceRepo);
    });
  ...
  it("skips validation entirely when no schema.json exists for the skill", async () => {
    ctx = createTestRepos();

    await given("the sanity-check skill has SKILL.md but no schema.json", () => {
      // createTestRepos already sets up sanity-check with just SKILL.md
    });
  ...
  it("handles malformed schema.json gracefully by skipping validation", async () => {
    ...
    await given("the sanity-check skill has a malformed schema.json (not valid JSON Schema)", () => {
      writeFileSync(
  ...
  it("warns when context_bundle is completely empty but required fields exist", async () => {
    ...
    await given("the sanity-check skill has a schema.json with 6 required fields", () => {
      seedSchemaJson(ctx.aliceRepo);
    });
  ...
  it("does not validate when skill has schema.json but context_bundle.required is absent", async () => {
    ctx = createTestRepos();

    await given("the sanity-check skill has a schema.json without a required array", () => {
  ...
  it("existing workflows work identically when schema.json is not present", async () => {
    ...
    await given("the sanity-check skill has only SKILL.md (no schema.json)", () => {

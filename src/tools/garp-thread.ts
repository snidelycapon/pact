/**
 * Handler for the garp_thread tool.
 *
...
export interface GarpThreadParams {
  thread_id: string;
}

export interface GarpThreadContext {
  userId: string;
  repoPath: string;
  git: GitPort;
  file: FilePort;
...
export interface GarpThreadResult {
  thread_id: string;
...
export async function handleGarpThread(
  params: GarpThreadParams,
  ctx: GarpThreadContext,
): Promise<GarpThreadResult> {

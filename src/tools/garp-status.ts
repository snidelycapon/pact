...
export interface GarpStatusParams {
  request_id: string;
}

export interface GarpStatusContext {
  userId: string;
  repoPath: string;
  git: GitPort;
  file: FilePort;
...
export async function handleGarpStatus(
  params: GarpStatusParams,
  ctx: GarpStatusContext,
): Promise<GarpStatusResult> {
  ...
  if (pendingFiles.includes(`${params.request_id}.json`)) {
    const request = parseRequestEnvelope(await ctx.file.readJSON<unknown>(`requests/pending/${params.request_id}.json`), params.request_id);
    return { status: "pending", request, response: undefined, ...(warning ? { warning } : {}) };
  }
  ...
  if (activeFiles.includes(`${params.request_id}.json`)) {
    const request = parseRequestEnvelope(await ctx.file.readJSON<unknown>(`requests/active/${params.request_id}.json`), params.request_id);
    return { status: "active", request, response: undefined, ...(warning ? { warning } : {}) };
  }

  // 4. Search completed/
  const completedFiles = await ctx.file.listDirectory("requests/completed");
  if (completedFiles.includes(`${params.request_id}.json`)) {
    const request = parseRequestEnvelope(await ctx.file.readJSON<unknown>(`requests/completed/${params.request_id}.json`), params.request_id);
    const response = parseResponseEnvelope(await ctx.file.readJSON<unknown>(`responses/${params.request_id}.json`), params.request_id);
    return { status: "completed", request, response, ...(warning ? { warning } : {}) };
  }

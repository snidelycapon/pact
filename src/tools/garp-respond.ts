/**
 * Handler for the garp_respond tool.
 *
...
export interface GarpRespondParams {
  request_id: string;
  response_bundle?: Record<string, unknown>;
}

export interface GarpRespondContext {
  userId: string;
  repoPath: string;
  git: GitPort;
  config: ConfigPort;
...
export async function handleGarpRespond(
  params: GarpRespondParams,
  ctx: GarpRespondContext,

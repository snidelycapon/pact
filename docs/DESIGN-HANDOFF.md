# DESIGN Wave Handoff — GARP Phase 2 Polish
...
```
/nw:design GARP Phase 2 Polish — design the architecture for 10 enhancement stories (US-009 through US-018) extending the existing GARP MCP server. Read the handoff below, then start with the key files.
```
...
Phase 2 adds 3 new tools, modifies 4 existing tools, adds 2 skill contracts, and 1 convention document.

### The 10 Stories (all DoR-validated, all have BDD scenarios)

| ID | Title | Type | Priority | Size |
|----|-------|------|----------|------|
| **US-010** | Auto-assign thread_id on garp_request | Modify existing | P1 | 1 day |
| **US-009** | garp_thread tool (view thread history) | New tool | P1 | 1-2 days |
...
| **US-013** | garp_cancel tool | New tool | P4 | 1-2 days |
| **US-014** | garp_amend tool | New tool | P4 | 1-2 days |
| **US-015** | Status field consistency fix | Modify existing | P4 | 0.5 days |
| **US-016** | Inbox auto-poll convention | Docs only | P5 | 0.5 days |
| **US-017** | Sanity-check skill contract | New SKILL.md | P6 | 0.5 days |
| **US-018** | Code-review skill contract | New SKILL.md | P6 | 0.5 days |
...
US-013 (garp_cancel) --+
US-014 (garp_amend)  --+--> US-015 (Status consistency)
...
- **Wave 3** (Skills + Convention): US-017, US-018, US-016
...
| `docs/requirements/us-009-garp-thread-tool.md` | garp_thread tool story + BDD |
...
| `docs/requirements/us-013-garp-cancel-tool.md` | garp_cancel story + BDD |
| `docs/requirements/us-014-garp-amend-tool.md` | garp_amend story + BDD |
...
| `docs/requirements/us-017-sanity-check-skill.md` | Sanity-check skill story |
| `docs/requirements/us-018-code-review-skill.md` | Code-review skill story |
| `docs/architecture/architecture.md` | Existing C4 architecture diagrams |
| `docs/adrs/` | 6 ADRs (git transport, single SKILL.md, stdio MCP, etc.) |
...
| `src/tools/garp-request.ts` | Request handler (thread_id + attachments already implemented) |
| `src/tools/garp-inbox.ts` | Inbox handler (short_id, thread_id, attachment_count already implemented) |
| `src/tools/garp-respond.ts` | Respond handler (moves pending → completed) |
| `src/tools/garp-status.ts` | Status handler (searches pending/active/completed) |
...
1. **Thread grouping algorithm** — threads where both parties send requests (e.g., design-skill rounds)
2. **Amendment visibility** — should inbox show an "amended" indicator?
3. **garp_thread output format** — full envelopes vs summary fields per entry
4. **Cancelled directory initialization** — .gitkeep in garp-init.sh or create on first cancel
5. **garp_cancel and garp_amend parameter schemas** — exact Zod definitions for MCP tool registration
6. **Cancelled status in GarpStatusResult** — add "cancelled" to the type union

# US-009: View Thread History (pact_thread)

## Problem (The Pain)
Cory is a developer engaged in a multi-round design-pact collaboration with Dan about a code-review pact. They exchanged 3 rounds over 2 days. Now Cory opens a new session and cannot remember what feedback Dan gave in round 2 or what the current draft looks like. The thread_id exists in the request JSON, but there is no tool to retrieve the conversation. Cory has to manually run pact_status on each request_id he can remember, hoping he has them all.
...
A new MCP tool `pact_thread` that, given a thread_id, returns all requests and responses in the thread ordered chronologically. Includes a thread summary with participant list, round count, and latest status.

## Domain Examples

### Example 1: Two-Round Design-Pact Thread
Cory and Dan have been designing a code-review pact. Round 1 (propose) was sent Monday, Dan responded with "needs_revision." Round 2 (refine) was sent Tuesday, still pending. Cory calls pact_thread with the thread_id and sees both rounds: round 1 complete with Dan's feedback, round 2 pending. He now knows exactly what Dan asked for and what his latest draft contains.

### Example 2: Single-Round Completed Thread
Cory sent a sanity-check to Dan last week. Dan responded the same day. Cory calls pact_thread to revisit the conversation. The thread shows 1 round, completed, with the original question and Dan's full response including evidence and recommendation.

### Example 3: Thread With No Matching Requests
Cory mistypes the thread_id (uses an old request_id that was never a thread anchor). pact_thread returns an empty result with the message "No requests found for this thread." Cory realizes his mistake and uses pact_inbox or pact_status to find the correct thread_id.
...
When Cory calls pact_thread with the thread_id
...
When Cory calls pact_thread with a thread_id
...
## Acceptance Criteria
- [ ] pact_thread accepts a thread_id parameter and returns all matching requests chronologically
...
## Technical Notes
- pact_thread scans pending/, completed/, and cancelled/ directories. It reads every JSON file in each directory and filters by thread_id. For the MVP scale (dozens of requests), sequential scan is fine. Indexing is a Tier 2 concern.
...
- Register the tool in mcp-server.ts following the same pattern as pact_status (pull, scan, return).

## Dependencies
- None (thread_id already exists in the schema and is written by pact_request)

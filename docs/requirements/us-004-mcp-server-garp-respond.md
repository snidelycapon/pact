# US-004: Submit a Response to a Request (garp_respond)
...
## Who (The User)
- Alex, colleague who has completed an investigation triggered by a GARP request
- Wants to send structured findings back to the requester
- Needs the response to be linked to the original request and follow the skill's response format

## Solution (What We Build)
An MCP tool `garp_respond` that accepts a structured response for a specific request_id, writes the response as a JSON file to `responses/`, moves the original request from `requests/pending/` (or `requests/active/`) to `requests/completed/`, and commits and pushes everything to the remote.

## Domain Examples

### Example 1: Respond to a Sanity-Check with Confirmation
Alex investigated Cory's memory leak question and confirmed it matches the ZD-4102 pattern. She composes a response with: answer ("YES -- same pattern"), evidence (compared refresh.ts with cleanup.ts), recommendation ("apply finally-block cleanup"). The agent presents this as a Plan for review. Alex approves. garp_respond writes `responses/req-20260221-143022-cory.json`, moves the request to `requests/completed/`, commits with message "[garp] response: req-20260221-143022-cory (sanity-check) alex -> cory", and pushes.
...
Alex tries to respond to req-001 but it has already been moved to `requests/completed/` (perhaps she responded in a different session and forgot). garp_respond returns an error: "Request req-001 is already completed." No duplicate response is written.
...
When the agent calls garp_respond with:
  | field           | value                                        |
...
And the changes are committed with message "[garp] response: req-20260221-001 (sanity-check) alex -> cory"
...
When the agent calls garp_respond for "req-20260221-001"
Then garp_respond returns an error indicating the request is already completed
...
And GARP_USER is set to "alex"
When the agent calls garp_respond for "req-20260221-001"
Then garp_respond returns an error indicating Alex is not the recipient
...
When the agent calls garp_respond for "req-nonexistent"
Then garp_respond returns an error indicating the request was not found
And no response is written
...
## Acceptance Criteria
- [ ] garp_respond writes a response JSON file to responses/{request_id}.json
...
- response_bundle is NOT validated by the MCP server -- same principle as context_bundle. The skill defines expected fields.
- The responder field is populated from GARP_USER env var and config.json lookup, not from tool input.

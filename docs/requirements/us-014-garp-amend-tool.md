# US-014: Amend a Pending Request (garp_amend)
...
A new MCP tool `garp_amend` that appends additional context to a pending request. The amendment is stored in an `amendments` array on the request envelope, preserving the original context_bundle. Only the original sender can amend.

## Domain Examples

### Example 1: Add Missing Ticket Reference
Cory sent a sanity-check to Dan but forgot the Zendesk ticket. He says "Add ZD-4521 to that request." The agent calls garp_amend with fields `{zendesk_ticket: "ZD-4521"}` and note "Added missing ticket reference." The request JSON now has the original context_bundle plus an amendments array with one entry. Dan sees both the original and the amendment.
...
Dan already responded to Cory's request. Cory tries to amend with additional context. garp_amend returns an error: "Request is already completed and cannot be amended." The completed request and response are preserved.
...
When Cory calls garp_amend for that request
Then garp_amend returns an error: "Request is already completed and cannot be amended"

### Scenario: Cannot amend a cancelled request
Given request "req-20260222-140000-cory-a1b2" was cancelled
When Cory calls garp_amend for that request
Then garp_amend returns an error: "Request is already cancelled and cannot be amended"

## Acceptance Criteria
- [ ] garp_amend appends an amendment entry to the request's amendments array
...
- [ ] Commit message follows convention: "[garp] amended: {request_id}"
- [ ] Changes are pushed to the remote

## Technical Notes
- Create src/tools/garp-amend.ts. The handler: git pull, find request in pending/, verify sender, read existing JSON, append to amendments array, write JSON, git add + commit + push.
...
- Should be implemented alongside US-013 (garp_cancel) as they share the sender-validation and pending-only gate patterns

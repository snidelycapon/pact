...
```
THREADS           ATTACHMENTS      LIFECYCLE         CONVENTION       SKILLS
========          ===========      =========         ==========       ======

US-010            US-012           US-013            US-016           US-017
Auto thread_id    Attachment       garp_cancel       Inbox            Sanity-check
(modify request)  inbox+status     (new tool)        auto-poll        skill
                  (modify tools)                     (docs only)
US-009                             US-014                             US-018
garp_thread                        garp_amend                         Code-review
(new tool)                         (new tool)                         skill
...
```
           ...
           |                     |
           v                     v
      US-009 (garp_thread)  US-011 (Thread-aware inbox)
           |
           v
      US-013 (garp_cancel) -----> US-015 (Status consistency)
           |                         ^
           |                         |
      US-014 (garp_amend) ----------+

      US-012 (Attachment inbox+status) -----> US-018 (Code-review skill)

      US-016 (Inbox auto-poll) — no dependencies

      US-017 (Sanity-check skill) — no dependencies
```
...
- Skills: US-017 (standalone), US-018 (after US-012)
...
|----|-------|------|-----------|-------------|----------|
| US-009 | garp_thread (View Thread) | 1-2 days | 5 | US-010 (beneficial) | P1 |
...
| US-013 | garp_cancel | 1-2 days | 5 | None | P4 |
| US-014 | garp_amend | 1-2 days | 5 | None | P4 |
| US-015 | Status field consistency | 0.5 days | 3 | US-013 | P4 |
| US-016 | Inbox auto-poll convention | 0.5 days | 3 | None | P5 |
| US-017 | Sanity-check skill | 0.5 days | 5 | None | P6 |
| US-018 | Code-review skill | 0.5 days | 5 | US-012 (beneficial) | P6 |
...
1. **US-010** (Auto thread_id) -- 1 day. Small change to garp_request, unlocks everything else.
2. **US-009** (garp_thread) -- 1-2 days. New tool, scan pattern similar to garp_status.
...
4. **US-013** (garp_cancel) -- 1-2 days. New tool.
5. **US-014** (garp_amend) -- 1-2 days. New tool. Parallelizable with US-013.
6. **US-015** (Status consistency) -- 0.5 days. Quick fix to garp_respond + apply to US-013.
7. **US-012** (Attachment inbox + status) -- 1 day. Modify existing tools.

### Wave 3: Skills + Convention (1-2 days)
8. **US-017** (Sanity-check skill) -- 0.5 days. Markdown only.
9. **US-018** (Code-review skill) -- 0.5 days. Markdown only.
...
**US-009 (garp_thread)**: Problem clearly states the pain of manually tracking thread_ids across sessions. 3 domain examples cover multi-round, single-round, and not-found. 5 scenarios including cancelled-request edge case. Technical notes specify scan strategy and tool registration. Size: 1-2 days for new tool following existing pattern.
...
**US-013 (garp_cancel)**: Problem uses a specific scenario (sent to wrong person). 3 examples cover wrong-person, stale, and cannot-cancel-completed. 5 scenarios. Technical notes reference garp-respond.ts as pattern. Size: 1-2 days for new tool.

**US-014 (garp_amend)**: Problem uses specific missing-context scenario. 3 examples cover single amendment, multiple, and cannot-amend-completed. 5 scenarios. Technical notes detail append-only strategy. Size: 1-2 days.
...
**US-017 (Sanity-check skill)**: Problem describes the Slack context-dump workflow it replaces. 3 examples with real customer data. 5 scenarios. Size: 0.5 days (markdown file).

**US-018 (Code-review skill)**: Problem describes unstructured PR review requests. 3 examples cover review request, approval, and changes-requested. 5 scenarios including multi-round and attachments. Size: 0.5 days (markdown file).
...
| Skill contracts too complex for agents to follow | MEDIUM | Start with sanity-check (simple) and code-review (moderate). Measure response schema compliance. | US-017, US-018 in real usage. |
...
- 3 new MCP tools: garp_thread, garp_cancel, garp_amend
- 2 modified tools: garp_request (auto thread_id), garp_inbox (thread grouping + attachments), garp_status (attachment paths + cancelled directory scan)
- 1 modified tool (bugfix): garp_respond (status field consistency)
- 2 new skill contracts: sanity-check, code-review
...
- Thread grouping algorithm: how to handle threads where some rounds are from different senders (e.g., design-skill where both parties send requests)
- Amendment visibility: should garp_inbox show an "amended" indicator for amended requests?
- garp_thread output format: full envelope vs summary fields for each entry
- Cancelled directory initialization: .gitkeep convention or created on first cancel
- garp_cancel and garp_amend parameter schemas (exact Zod definitions)
- Whether to add `cancelled` to the GarpStatusResult type union
...
| docs/ux/phase2-polish/journey-skill-contracts-visual.md | New skill contracts overview |

### Requirements Produced
| File | Contents |
|------|----------|
| docs/requirements/us-009-garp-thread-tool.md | garp_thread tool |
| docs/requirements/us-010-auto-thread-id.md | Auto thread_id on garp_request |
| docs/requirements/us-011-thread-aware-inbox.md | Thread-aware garp_inbox |
| docs/requirements/us-012-attachment-inbox-paths.md | Attachment details in inbox + status |
| docs/requirements/us-013-garp-cancel-tool.md | garp_cancel tool |
| docs/requirements/us-014-garp-amend-tool.md | garp_amend tool |
...
| docs/requirements/us-017-sanity-check-skill.md | Sanity-check SKILL.md |
| docs/requirements/us-018-code-review-skill.md | Code-review SKILL.md |

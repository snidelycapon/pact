# US-018: Code-Review Skill Contract
...
A code-review SKILL.md that defines the context bundle fields (repository, branch, language, description, areas_of_concern, related_tickets) and response structure (status, summary, blocking_feedback, advisory_feedback, questions). The skill encourages attaching the diff and test results as files.
...
And the agent loads skills/code-review/SKILL.md
...
And the agent attaches the diff file using garp_request's attachment parameter

### Scenario: Reviewer receives structured review context
Given Dan receives a code-review request from Cory
When Dan's agent loads the request and SKILL.md
Then Dan sees: what repo and branch, what language, what changed and why, specific areas of concern
And the agent can read the attached diff file via the path from garp_status
...
### Scenario: Skill contract documents expected attachments
Given a developer reads skills/code-review/SKILL.md
Then the skill includes an "Expected Attachments" section
And it lists diff file and test results as recommended attachments

## Acceptance Criteria
- [ ] SKILL.md committed to examples/skills/code-review/
...
- [ ] At least 1 worked example in the SKILL.md
- [ ] Skill exercises attachment feature (diff file as attachment)

## Technical Notes
- Place at examples/skills/code-review/SKILL.md.
- Follow the same structure as design-skill/SKILL.md (the most complex existing skill).
- The "Expected Attachments" section is a new convention for skills. It tells the sender's agent what files to attach and tells the receiver what to expect. This convention should be documented so future skill authors can use it.
...
- The skill itself has no code dependencies -- it is a markdown file

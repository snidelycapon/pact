# US-013: Cancel a Pending Request (garp_cancel)
...
A new MCP tool `garp_cancel` that moves a pending request to requests/cancelled/, updates the status field to "cancelled", and commits with a structured message. Only the original sender can cancel.

## Domain Examples

### Example 1: Cancel Sent to Wrong Person
Cory sent request "req-20260222-140000-cory-a1b2" (code-review) to Dan. He immediately realizes Dan does not work on that repo -- it should go to Maria Santos. Cory says "Cancel that code review request." The agent calls garp_cancel, the request moves to cancelled/, and Cory resends to Maria Santos.
...
Cory tries to cancel request "req-20260222-140000-cory-a1b2" but Dan already responded. The request is in completed/. garp_cancel returns an error: "Request is already completed and cannot be cancelled." The completed request and Dan's response are preserved.
...
When Cory calls garp_cancel for that request
Then garp_cancel returns an error: "Request is already cancelled"
...
## Acceptance Criteria
- [ ] garp_cancel moves a pending request to requests/cancelled/
- [ ] Status field in the JSON is updated to "cancelled" before the move
- [ ] Only the original sender (sender.user_id == GARP_USER) can cancel
- [ ] Returns an error for completed, already-cancelled, or non-existent requests
- [ ] Commit message follows convention: "[garp] cancelled: {request_id}"
- [ ] Changes are pushed to the remote
- [ ] Cancelled requests are visible via garp_status with status "cancelled"

## Technical Notes
- Create src/tools/garp-cancel.ts following the same pattern as garp-respond.ts.
...
- garp_status (src/tools/garp-status.ts) needs to be updated to also scan requests/cancelled/ directory. Currently it scans pending, active, completed.

## Dependencies
- US-015 (repo structure update for cancelled/ directory) -- can be combined into this story or done as a prerequisite
- US-009 (garp_thread) should handle cancelled requests in thread history

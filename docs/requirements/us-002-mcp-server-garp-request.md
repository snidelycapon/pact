# US-002: Submit a GARP Request (garp_request)
...
An MCP tool `garp_request` that accepts a structured request (rigid envelope with flexible context_bundle), validates the envelope, writes it as a JSON file to `requests/pending/` in the GARP repo, and commits and pushes to the remote. The agent composes the request with guidance from the SKILL.md and presents it for human review before submission.

## Domain Examples

### Example 1: Sanity-Check Request for a Memory Leak
Cory is investigating a memory leak in Acme Corp's Platform v3.2. His agent has examined `src/auth/refresh.ts` lines 45-90 and found that refresh tokens are not being garbage collected. Cory says "Send a sanity check to Alex about this." The agent loads the sanity-check SKILL.md, assembles the context bundle (customer: Acme Corp, product: Platform v3.2, files: refresh.ts:L45-90, finding: tokens not GC'd, question: "Does this match the session service pattern from last month?"), presents the request for review, and on approval calls garp_request. The file `requests/pending/req-20260221-143022-cory.json` is created, committed, and pushed.
...
Given Cory has a configured MCP server with GARP_USER "cory" and GARP_REPO pointing to the acme-garp repo
And config.json lists "alex" as a team member
When the agent calls garp_request with:
  | field         | value                                              |
...
And the file is committed with message "[garp] new request: {id} (sanity-check) -> alex"
...
Then garp_request returns an error indicating "bob" is not a team member
...
Given Cory's agent calls garp_request
...
Given the agent calls garp_request missing the "recipient" field
Then garp_request returns a validation error listing the missing field
And no file is created

## Acceptance Criteria
- [ ] garp_request writes a valid JSON file to requests/pending/ with unique request_id
...
- context_bundle is NOT validated by the MCP server -- it is a flexible payload. The skill defines expected fields, but the server is type-agnostic.
- The `sender` field is populated from GARP_USER env var and config.json lookup, not from tool input (prevents spoofing).

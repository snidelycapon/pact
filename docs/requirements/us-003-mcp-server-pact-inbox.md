# US-003: Check PACT Inbox (pact_inbox)
...
## Who (The User)
- Alex, colleague receiving PACT requests
...
An MCP tool `pact_inbox` that pulls the latest from the PACT repo, scans `requests/pending/` for files where the recipient matches the current user, and returns a list of pending requests with summary information.

## Domain Examples

### Example 1: One Pending Sanity-Check Request
Alex starts a new Craft Agents session and says "Check my PACT inbox." The agent calls pact_inbox. The tool runs `git pull`, scans `requests/pending/`, finds `req-20260221-143022-cory.json` addressed to user_id "alex" with type "sanity-check" from Cory. Returns: 1 pending request with ID, type, sender display name, creation timestamp, and the question from the context bundle.

### Example 2: Empty Inbox
Alex checks her inbox but no one has sent her anything. pact_inbox returns 0 pending requests. The agent reports "No pending requests in your inbox."

### Example 3: Multiple Requests from Different Senders
Alex has two pending requests: one sanity-check from Cory about a memory leak, and another sanity-check from Dana about a deployment issue. pact_inbox returns both, ordered by creation time (oldest first). Alex can see at a glance what each one is about and decide which to handle first.

## UAT Scenarios (BDD)

### Scenario: Retrieve pending requests for the current user
Given the PACT repo contains:
...
When the agent calls pact_inbox
...
Given the PACT repo has no pending requests addressed to "alex"
And PACT_USER is set to "alex"
When the agent calls pact_inbox
Then the tool runs git pull
...
Given PACT_USER is set to "alex"
...
## Acceptance Criteria
- [ ] pact_inbox runs git pull before scanning
- [ ] Only returns requests where recipient.user_id matches PACT_USER
...
## Technical Notes
- The summary field extracted from context_bundle is best-effort: try `question`, then `issue_summary`, then first 200 chars of the bundle. The MCP server does not know the pact schema.
- git pull should have a timeout to avoid hanging the tool call when the network is slow.
- pact_inbox does NOT move files or change state -- it is a read-only operation.
- Consider including the pact file content in the response payload so the agent does not need a separate file read to auto-load the pact. This is a DESIGN wave decision.

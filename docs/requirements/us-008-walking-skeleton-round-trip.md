...
- Both using Craft Agents with the GARP MCP server configured
- Testing the complete end-to-end flow for the first time

## Solution (What We Build)
A walking skeleton that validates the complete coordination loop: Cory composes a request, pushes to the shared repo, Alex pulls and sees it in her inbox, investigates, responds and pushes back, Cory pulls and sees the response. Every piece of the architecture is exercised: MCP tools, git operations, request/response JSON, skill loading, directory lifecycle.

## Domain Examples

### Example 1: The Happy Path Round-Trip
Cory is investigating the Acme Corp memory leak. He tells his agent to send a sanity check to Alex. The agent loads the skill, assembles the context, presents it for review, Cory approves, and it pushes. Minutes later, Alex checks her inbox, sees the request, opens it with full context loaded, investigates, composes a response, approves, and pushes. Cory checks status in a new session and sees Alex's findings: "Yes, same pattern as ZD-4102. Apply the finally-block cleanup."
...
During the round-trip, both Cory and Alex make other commits to the repo (new requests, other responses). The GARP tool handles git pull/push correctly with no conflicts, no lost data, and structured commit messages that make the git log readable as an audit trail.

## UAT Scenarios (BDD)

### Scenario: Complete round-trip from compose to response
Given Cory and Alex both have configured MCP servers for the shared "acme-garp" repo
And the sanity-check SKILL.md exists in the repo
...
And Alex checks her GARP inbox
...
And the sanity-check SKILL.md is loaded
...
When someone runs "git log --oneline" on the GARP repo
Then the log contains commits for:
  | message pattern                                          |
  | [garp] new request: req-20260221-001 (sanity-check) -> alex |
  | [garp] response: req-20260221-001 (sanity-check) alex -> cory |
...
When Cory's agent runs garp_status
...
- [ ] Both users' agents successfully load the skill file and produce structured content
...
- Success here validates hypothesis H1 (git round-trip works), H3 (git as transport), and partially validates H2 (skill contracts produce consistent behavior).
...
- After this validates, the team should run 5+ additional round-trips to test skill contract consistency (discovery risk R1).

## Dependencies
- US-001: Repo structure must exist
- US-002: garp_request must work
- US-003: garp_inbox must work
- US-004: garp_respond must work
- US-005: garp_status must work
- US-006: Sanity-check SKILL.md must exist

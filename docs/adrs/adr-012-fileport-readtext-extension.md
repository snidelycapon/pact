...
The skill metadata module (ADR-010) needs to read SKILL.md files as plain text and check whether schema.json exists before attempting to read it. The current FilePort interface has:
...
There is no `readText(path)` (only writeText exists) and no `fileExists(path)`. The existing code uses Node's `existsSync` directly in `garp-request.ts` to check skill existence, bypassing the port boundary.
...
Also replace the direct `existsSync` call in `garp-request.ts` with `file.fileExists()` to fix the existing port boundary violation.

## Alternatives Considered

### Use readJSON for SKILL.md by Wrapping in Try/Catch

Call `readJSON` on SKILL.md, catch the JSON parse error, and use the raw string from the error context.
...
### Separate SkillFilePort

Create a new port specifically for skill file operations.
...
- **Rejection rationale**: readText and fileExists are generic file operations, not skill-specific. They belong on FilePort.
...
- fileExists replaces the port boundary violation in garp-request.ts

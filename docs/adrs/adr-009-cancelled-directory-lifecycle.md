...
Add `requests/cancelled/` with `.gitkeep` to `pact-init.sh`, matching the existing convention for `pending/`, `active/`, and `completed/`. The directory exists from repo creation. pact_status scans this directory alongside the other three. pact_cancel moves files from `pending/` to `cancelled/` using the same `git mv` pattern as pact_respond.

Status field consistency (US-015): pact_cancel sets `status: "cancelled"` in the envelope JSON before moving. pact_respond sets `status: "completed"` before moving. The JSON status field always matches the directory.

## Alternatives Considered

### Create on First Cancel

Do not add `cancelled/` to pact-init.sh. Instead, create `requests/cancelled/` on the first pact_cancel invocation using `mkdir -p`.

- **Pro**: Fewer directories in a fresh repo. No migration needed for existing repos (directory appears naturally).
- **Con**: `pact_status` and `pact_thread` must handle the case where `cancelled/` does not exist. Every `listDirectory("requests/cancelled")` call would need a try-catch or existence check. The FileAdapter currently throws on missing directories. This adds defensive code throughout the codebase.
...
- **Pro**: No new directory. No git mv. pact_cancel just rewrites the JSON.
...
- pact_status scans 4 directories without conditional logic

...
### WS-1: Discovery (garp-discover.test.ts)

**User goal**: An agent discovers available request types and team members so it knows what it can do and who it can reach.

**Scenario**: "Agent discovers available request types and team members from YAML skill contracts"

**Journey**:
1. Skills installed as YAML frontmatter SKILL.md files (3 skills: ask, code-review, sanity-check)
2. Agent calls `garp_discover` with no query
3. Response includes all 3 skills sorted alphabetically with structured metadata
4. Response includes team members (alice, bob)

**Observable outcomes**:
- Skills array has 3 entries with correct names
- Each skill has description and when_to_use parsed from YAML frontmatter
- Each skill has context_bundle with required fields and field definitions (type + description)
- Team array has 2 members with user_id and display_name

**Why this is user-centric**: An agent receiving this response has everything needed to compose a request — it knows the skills, the fields, and the recipients. One round-trip to go from "I know nothing" to "I can act."

### WS-2: Action Dispatch (garp-do.test.ts)

**User goal**: An agent sends a request to a teammate, and the recipient can see it in their inbox — all through `garp_do`.

**Scenario**: "Agent sends a request to a teammate and the recipient sees it in their inbox"

**Journey**:
1. Skills installed as YAML frontmatter format
2. Alice's agent calls `garp_do({ action: "send", ... })` with a sanity-check request to Bob
3. Result confirms pending status and returns request_id
4. Request file created in `requests/pending/` with correct envelope
5. Bob's agent calls `garp_do({ action: "inbox" })`
...
- **Stakeholder-demonstrable?** Yes — "The agent discovers 3 skills and 2 team members" / "Alice sends Bob a request and he sees it"
...
- **Through driving ports?** Yes — `createGarpServer().callTool()` is the hexagonal boundary

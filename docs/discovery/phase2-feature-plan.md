# Phase 2 Feature Plan — GARP Post-MVP
...
- Create 2-3 new skill contracts that test the protocol's flexibility
...
**Why highest priority**: thread_id exists as a schema primitive but has zero tooling. The design-skill contract documents a multi-round workflow that depends on thread tracking, but users must manually remember thread IDs. This is the gap between "protocol supports it" and "users can actually use it."

**Features**:

| Feature | Description | Scope |
|---------|------------|-------|
| F1.1: garp_thread tool | New MCP tool. Given a thread_id, return all requests in the thread ordered chronologically with their responses. Include thread summary (participant list, round count, latest status). | NEW TOOL |
| F1.2: Thread-aware inbox | Update garp_inbox to group entries by thread_id. Threaded entries show as a single inbox item with round count, rather than N separate items. | MODIFY garp_inbox |
| F1.3: Auto thread_id on round 1 | When a request is the first in a thread (no thread_id provided), set thread_id = request_id automatically. The design-skill contract documents this convention manually; it should be automatic. | MODIFY garp_request |

**Acceptance criteria**:
- garp_thread returns chronological request+response pairs for a given thread
- Inbox groups threaded requests, showing "Thread: design-skill (3 rounds)" instead of 3 separate entries
...
|--------|------------|-----------------|
| A2.1: Create tech support skill | Write a "sanity-check" or "bug-handoff" SKILL.md based on the original discovery workflow description. Include fields for customer, product, repos, files, investigation, question, zendesk_ticket. | Skill file committed to skills/ |
...
| A2.4: Iterate the skill | Based on the real handoff, refine the skill contract. What fields were missing? What was unnecessary? | Revised skill committed |
...
| Feature | Description | Scope |
|---------|------------|-------|
| F3.1: Attachment paths in inbox | Update garp_inbox entries to include the absolute paths to attachment files, not just the count. | MODIFY garp_inbox |
| F3.2: Attachment content in status | Update garp_status to include attachment metadata and file paths when displaying a request. | MODIFY garp_status |
| F3.3: Skill attachment expectations | Extend SKILL.md convention to include an "Expected Attachments" section where skill authors can specify what files should be attached. | CONVENTION |

**Acceptance criteria**:
- Receiver's inbox entry includes `attachment_paths: ["/path/to/attachments/req-id/filename"]`
- garp_status shows attachment list with descriptions and paths
- At least 1 skill has attachment expectations documented
...
| Feature | Description | Scope |
|---------|------------|-------|
| F4.1: garp_cancel tool | New MCP tool. Sender can cancel a pending request. Moves request to cancelled/ directory. Only the sender can cancel. | NEW TOOL + NEW DIRECTORY |
| F4.2: garp_amend tool | New MCP tool. Sender can append additional context to a pending request. Writes an amendment entry to the context bundle (e.g., `amendments: [{added_at, fields}]`). Does not overwrite original context. | NEW TOOL |
| F4.3: Status field consistency | When garp_respond moves a request to completed/, update the status field in the JSON to "completed". Same for cancel -> "cancelled". | MODIFY garp_respond, NEW garp_cancel |

**Acceptance criteria**:
- garp_cancel moves pending request to cancelled/, commits with `[garp] cancelled:` prefix
- garp_amend appends to existing request without overwriting, commits with `[garp] amended:` prefix
...
| Feature | Description | Scope |
|---------|------------|-------|
| F5.1: Session-start inbox check | Document the pattern for MCP hosts to auto-invoke garp_inbox when a session starts. For Claude Code, this is a system prompt instruction. For Craft Agents, this could be a hook. | DOCUMENTATION / CONVENTION |

**Acceptance criteria**:
- Documentation describes how to configure auto-inbox-check for at least 2 MCP hosts
- The pattern is non-invasive (does not require code changes to GARP itself)

**Risk**: VERY LOW -- this is a usage convention, not a code change.

---

### Priority 6: New Skill Contracts

**Source opportunity**: Cross-cutting (O1, O4, O7)
**Why this priority**: The protocol's flexibility is only validated by the diversity of skills that use it. Two skills (ask, design-skill) is a start; the protocol should handle meaningfully different workflow patterns.

**Candidates**:

| Skill | Pattern | Why Interesting |
|-------|---------|----------------|
| code-review | Ping-pong | The design-skill SKILL.md already sketches this. Rich context (diff URL, language, areas of concern). Tests attachment feature with diff files. |
...
| standup | Broadcast (manual) | Sender asks multiple recipients the same question. Tests one-to-many pattern. Reveals whether garp_request needs a multi-recipient option or if N separate requests is acceptable. |
...
**Acceptance criteria**:
- 2+ new skills committed to examples/skills/
- At least 1 skill exercises attachments
- At least 1 skill exercises thread_id for multi-round

**Risk**: LOW -- skill authoring is markdown. The interesting risk is whether agents follow complex skills reliably.
...
| 2 | Complex skill contracts produce consistent agent behavior | MED | "ask" is trivially simple. "design-skill" has not been tested with two real humans. More complex skills (code-review, sanity-check) may exceed SKILL.md's ability to guide agent behavior. | Create 2 complex skills, run 5+ round-trips each, measure response schema compliance. |
| 3 | thread_id is sufficient for multi-round orchestration | MED | thread_id is a flat string. It does not enforce ordering, track round numbers, or provide thread state. The design-skill contract manually tracks round numbers in the context bundle. Will this scale? | Build thread tools (F1.1-F1.3), exercise with 3+ multi-round threads. |
...
**New MCP tools**: 3 (garp_thread, garp_cancel, garp_amend)
**Modified tools**: 2 (garp_inbox, garp_request)
**New skills**: 2-4 (sanity-check, code-review, and optionally standup, incident-handoff)
...
5. Phase 2 scope is contained: 3 new tools, 2 modified tools, 2-4 skills, no infrastructure changes
6. No blocking dependencies on external systems or users

**Conditions for GO**:

1. Complete A2.1 (tech support skill) before building thread/lifecycle tools -- validate the value proposition first
...
- [ ] A2.1 (tech support skill) completed as validation prerequisite

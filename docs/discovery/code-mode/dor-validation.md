# Definition of Ready Validation -- GARP Code Mode Phase A
...
- Cory cannot remember which skill type to use ("was it code-review or sanity-check?")
- Maria Santos is new and has no way to discover the catalog
Both are described in domain language (skills, request types, SKILL.md) without technical jargon. The scaling consequence is quantified: 1,340 tokens at 4 skills, 33,600 at 100 skills.

### 2. User/persona identified with specific characteristics
**PASS**
- Cory: developer composing a GARP request, knows the system but cannot remember exact skill names
- Maria Santos: new team member, has never seen the skill catalog, needs guided discovery
...
**PASS** (3 examples)
- Example 1: Cory lists all skills, agent selects "code-review" for "Can someone review my auth changes?" -- includes full catalog listing with real skill names and field lists
...
**PASS** (7 scenarios)
1. List all available skills (happy path)
...
6. Falls back to SKILL.md (backward compat)
...
- Returns all skills when no query (from scenario 1)
...
**PASS**
Estimated 2-3 days. 7 scenarios. Single tool following existing patterns (similar to garp_thread or garp_status). Demonstrable in a single session: call garp_skills, see results.
...
### 1. Problem statement clear and in domain language
**PASS**
Problem is highly specific: "three additional file reads totaling 221 lines of markdown just to understand what each request expects." Quantifies the exact token cost (66 + 128 + 27 lines). Describes the user's flow (check inbox, read SKILL.md for each type, then respond).
...
- Example 3: ask skill without schema.json still gets enrichment from SKILL.md parsing

### 4. UAT scenarios in Given/When/Then (3-7 scenarios)
**PASS** (5 scenarios)
1. Inbox entry includes skill description
2. Inbox entry includes response field names
3. Enrichment uses schema.json when available
4. Falls back to SKILL.md
5. Graceful degradation when skill missing

### 5. Acceptance criteria derived from UAT
**PASS**
5 acceptance criteria, each traceable:
- skill_description included (from scenario 1)
...
### 6. Story right-sized (1-3 days, 3-7 scenarios)
**PASS**
Estimated 1 day. 5 scenarios. Modifies existing tool (garp_inbox) with 2 new fields. Demonstrable in a single session: call garp_inbox, see enriched entries.

### 7. Technical notes identify constraints and dependencies
**PASS**
Notes specify: InboxEntry interface modification, shared module reuse, graceful degradation on missing skills, caching per request_type, InboxThreadGroup update.

### 8. Dependencies resolved or tracked
**PASS**
US-019 noted as beneficial (shared parsing module). US-021 noted as beneficial (schema.json). Both are non-blocking -- enrichment works with SKILL.md parsing alone.
...
- Skill authors: want contracts followed accurately
...
- Example 3: Ask skill without schema.json works unchanged -- demonstrates backward compatibility
...
- schema.json for all 4 skills (from scenarios 6-7)
- JSON Schema draft 2020-12 (from scenario 1)
- additionalProperties: true (from scenario 5)
- Validation in garp_request (from scenario 2)
- Warnings not rejections (from scenario 2)
- No validation without schema (from scenario 4)
- Alignment with SKILL.md (from scenarios 6-7)

### 6. Story right-sized (1-3 days, 3-7 scenarios)
**PASS**
Estimated 1-2 days. 7 scenarios. Deliverables: 4 schema.json files + validation logic in garp_request. Demonstrable in a single session: submit request with missing fields, see warnings.

### 7. Technical notes identify constraints and dependencies
**PASS**
Notes specify: file location, validation placement in garp_request.ts, key-presence-only validation strategy, return type change, additionalProperties rationale, manual authoring for 4 skills, version strategy deferred to Phase B.

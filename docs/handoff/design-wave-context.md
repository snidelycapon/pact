...
A unified PACT architecture where:

1. **Collapsed tool surface** — Instead of N enumerated MCP tools (currently 8, projected 30+), agents discover capabilities through a small API surface and compose requests against it. Token cost stays O(1) regardless of pact count.

2. **Declarative brain processing** — PACT.md contracts include optional `hooks` sections that define validation, enrichment, routing, and auto-response rules. Serverless functions triggered by git events execute these rules. No daemon, no persistent state.

3. **Pacts directory as the single source of truth** — Both client-side discovery and server-side brain processing are driven by the same pacts. The brain doesn't need a separate config layer; it reads the same PACT.md that agents and humans use.
...
- **Pact pattern works**: Rigid envelope + flexible payload, with PACT.md defining per-type behavior.
- **Two users confirmed**: Cory + Dan completed real round-trips with zero failures.

### From Code Mode Discovery (Phase A)

- **Context pressure grows O(tools + pacts)**: At 7 tools + 4 pacts = ~2,840 tokens (fine). At 30 tools + 100 pacts = ~40,000 tokens = 20% of context (not fine).
- **No discovery mechanism exists**: Agents must know pact names upfront. Works at 4 pacts, breaks at 20+.
- **Pacts are human-readable only**: PACT.md content is never parsed by the server beyond checking file existence.
...
- **Brain should be stateless serverless functions**: Triggered by git events, execute declarative rules from PACT.md, write results back to git, terminate.
- **Git remains canonical**: No separate database, no sync layer, no dual-backend.
- **Pact-specific, not global**: Each pact defines its own brain processing (or none). New behavior = new pact, zero infra code changes.
...
| File | Lines | Purpose |
|------|-------|---------|
| `src/tools/pact-request.ts` | 118 | Submit request: validate, build envelope, write to pending/, commit, push |
| `src/tools/pact-inbox.ts` | 193 | Read pending/: filter by recipient, thread grouping, pact enrichment |
| `src/tools/pact-respond.ts` | 98 | Respond: write response, mv pending→completed, commit, push |
| `src/tools/pact-status.ts` | 126 | Read: search all dirs for request, return status + envelope |
| `src/tools/pact-thread.ts` | 161 | Read: scan all dirs for thread_id, pair with responses |
| `src/tools/pact-cancel.ts` | 58 | Cancel: validate sender, mv pending→cancelled, commit, push |
| `src/tools/pact-amend.ts` | 76 | Amend: validate sender, append amendment, rewrite, commit, push |
...
| `src/server.ts` | 125 | Test-facing factory: `createPactServer` → `callTool` dispatch |

### Phase A Additions (Disposable — to be rethought)

| File | Lines | Purpose | Disposition |
|------|-------|---------|-------------|
| `src/pact-parser.ts` | 291 | PACT.md + schema.json parsing (57 surviving mutants, 63% mutation score) | **Rip out and redesign** |
| `src/tools/pact-pacts.ts` | 82 | MCP tool: list/search pacts with metadata | **Rip out — replaced by collapsed discovery** |

### Phase A Modifications to Existing Files (Review)

| File | What Changed | Disposition |
|------|-------------|-------------|
| `pact-request.ts` | Schema validation via pact-parser → `validation_warnings` | **Keep the WARN pattern, rethink implementation** |
| `pact-inbox.ts` | Pact enrichment: `pact_description`, `response_fields`, per-invocation cache | **Remove enrichment coupling to old parser** |
| `mcp-server.ts` | Registered `pact_pacts` as 8th tool | **Will be rewritten for collapsed surface** |
...
### Pacts (Redesign format)

| File | Purpose | Disposition |
|------|---------|-------------|
| `examples/pacts/ask/PACT.md` | Simple Q&A pact | **Rewrite for new format** |
| `examples/pacts/design-pact/PACT.md` | Multi-round design workflow | **Rewrite for new format** |
| `examples/pacts/sanity-check/PACT.md` | Tech support handoff | **Rewrite for new format** |
| `examples/pacts/code-review/PACT.md` | Code review with attachments | **Rewrite for new format** |
| `examples/pacts/*/schema.json` | JSON Schema companions (Phase A) | **Evaluate — may merge into PACT.md format** |
...
- **Git transport** — pull-rebase-retry, atomic commits, `[pact]` commit tags
...
- **pact-parser.ts** — Heuristic markdown parsing. 63% mutation score. Replace with structured format parsing.
- **pact-pacts.ts** — Enumerated pact listing tool. Replace with collapsed discovery API.
- **schema.json as separate files** — May merge into a structured PACT.md format (YAML frontmatter).
- **Inbox pact enrichment** — Coupled to old parser. Rethink as part of collapsed tool response.
...
**Target**: 2-3 meta-tools with fixed token cost regardless of pact/capability count.

Candidates:
- **`pact_discover`** — "What can I do?" Query pacts, team members, threads. Returns just enough for the agent to compose a request.
- **`pact_do`** — "Do this." Accepts a structured action (send request, respond, cancel, amend, check status, view thread) and dispatches to the right handler.
...
### Declarative Brain in PACT.md

Each pact optionally includes brain processing rules:
...
2. Loads the pact for the request type
...
**Key constraint**: The brain uses the same pact format as client agents. No separate brain config.

### Unified Pact Format

The current split (PACT.md for humans + schema.json for machines + hooks for server) should converge into a single structured format that serves all three consumers:
...
4. **What replaces PACT.md + schema.json?** Single structured file? YAML frontmatter + markdown?
5. **How does hooks integrate?** Same file? Separate file in the pact directory?
6. **How is the pact format parsed?** YAML parser (deterministic) vs heuristic markdown parsing (what we're nuking)?
...
10. **Error handling**: Comment on request + notify pact author? Something else?
...
| ADR | Decision | Status |
|-----|----------|--------|
| ADR-010 | Pact metadata at application core, not a new port | **Superseded** — pact-parser being replaced |
...
| 2 | Complex pacts produce consistent agent behavior | MED | **NOT TESTED** at scale |
...
| 5 | PACT.md format can serve agents, humans, and brain simultaneously | MED | **HYPOTHESIS** |
...
1. Collapses PACT's 8 MCP tools into 2-3 meta-tools for O(1) context cost
2. Replaces the heuristic pact-parser with a structured, deterministic pact format
3. Makes that pact format serve three consumers: agents (discovery), humans (readability), brain (processing rules)
...
**You are NOT:**
- Maintaining backward compatibility with Phase A's pact-parser or pact_pacts tool

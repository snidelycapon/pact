...
```
 ...
 [1] Alex starts a session (or checks inbox)
     ...
     |
     v
 [2] Agent calls garp_inbox              <──────  requests/pending/
 ...
 [4] Alex opens the request
     ...
     |
     v
 [5] Agent loads the full request + SKILL.md
     The SKILL.md auto-loads based on request_type
     |
     | The agent now has:
     | - The skill contract (how to handle this type)
 ...
 [7] Alex investigates with agent assistance
     ...
     |
     | The SKILL.md is guiding the agent's understanding
 ...
 [9] Agent composes response
     Uses SKILL.md guidance for response structure:
 ...
 [10] Response presented Plan-style
     ...
     v
 [11] Agent calls garp_respond             ──────>  responses/
...
| 2 | Agent pulls and scans | garp_inbox | - | requests/pending/*.json |
| 3 | Inbox listing displayed | Agent output | Aware | Inbox listing |
| 4 | Opens specific request | Natural language | Engaged | - |
| 5 | Agent loads request + SKILL.md | Auto-load on request type | - | SKILL.md + request JSON |
...
| 9 | Agent composes response | SKILL.md guided | - | Response draft |
| 10 | Response shown Plan-style | Plan submission UI | Confident | Response JSON (draft) |
| 11 | Response approved and pushed | garp_respond | Done | responses/req-*.json |
...
- The SKILL.md loads automatically based on request type
...
### Same Skill File, Both Sides (Step 5)
Alex loads the SAME SKILL.md that Cory used to compose the request. This means:
- Both sides understand the same contract
- The skill defines both "how to compose" and "how to respond"
- Alex's agent knows what response fields are expected because the skill says so
- No misalignment between what sender sent and what receiver expects

### Normal Investigation Mode (Step 7)
The investigation itself is a normal agent conversation. Alex is not in a special "coordination mode" — they are just doing their usual work, but with better starting context. The skill provides guidance, not constraints.
...
2. **Step 5 — Skill auto-loading**: If the request type references a skill that does not exist locally (repo out of sync), the agent cannot load it.
   - Mitigation: garp_inbox runs git pull first, which syncs skills

3. **Step 7 — Local file access**: The context bundle references files in repos Alex may not have cloned (e.g., platform-auth).
   - Mitigation: This is a human decision — Alex needs access to the relevant repos. The GARP does not solve repo access, just context delivery.
...
Key difference: Beads is local multi-agent within one machine. This GARP is distributed across machines with git as transport, and HITL is mandatory at every node (not optional).

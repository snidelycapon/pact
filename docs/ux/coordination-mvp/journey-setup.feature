Feature: Setup and Onboarding
  As a new team member joining a PACT workspace
  I want to get from zero to a working PACT setup
  So that I can send and receive structured requests with my team

  Background:
    Given a private GitHub repo "acme-pact" exists with the standard directory structure
    And the repo contains a pact file at "pacts/sanity-check/PACT.md"
    And Cory has a working local MCP server configured for "acme-pact"

  # --- Onboarding Journey ---

  Scenario: Alex clones the repo and configures MCP source
    Given Alex has accepted the GitHub invitation for "acme-pact"
    When Alex clones the repo to their local machine
    And Alex adds a PACT MCP source in Craft Agents with:
      | setting    | value                        |
      | PACT_REPO | /Users/alex/repos/acme-pact |
      | PACT_USER | alex                         |
  ...
  Scenario: Alex sees a pre-seeded welcome request in their inbox
    ...
    And Alex has a configured MCP source for "acme-pact"
  ...
  Scenario: Alex responds to the welcome request completing the round-trip
    ...
    Then the agent loads "pacts/sanity-check/PACT.md" for response guidance
    And the agent composes a response and presents it for review
    And Alex approves the response
    And the agent calls pact_respond with the approved response
  ...
  Scenario: Cory verifies the round-trip by checking status
    ...
    Then the agent calls pact_status
  ...
  Scenario: MCP server rejects invalid repo path
    Given Alex tries to configure MCP source with PACT_REPO "/nonexistent/path"
  ...
  Scenario: Inbox check with network unavailable falls back to local state
    Given Alex has a configured MCP source for "acme-pact"

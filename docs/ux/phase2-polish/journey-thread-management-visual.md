...
## Actors
- **Cory** — Sender engaged in a multi-round design-pact collaboration
...
```
 ...
 [1] Cory sends design-pact request (round 1)
     pact_request(type: "design-pact",
                  recipient: "dan",
                  context_bundle: {pact_name: "code-review",
                                   round: 1, phase: "propose"})
     |
     | No thread_id provided -- this is round 1
     v
 [2] pact_request auto-assigns thread_id    ──>  requests/pending/
 ...
 [3] Dan responds to round 1                ──>  responses/
     pact_respond(request_id: "...-a1b2",          req-20260222-100000-cory-a1b2.json
 ...
 [5] Cory wants to see the full conversation
     ...
     |
     v
 [6] Agent calls pact_thread               <──  Scans ALL directories:
 ...
 [7] Thread view displayed
     ...
     |  Pact: design-pact | Rounds: 2 | Participants: Cory, Dan|
     +----------------------------------------------------------+
     |                                                            |
     |  Round 1 — 2026-02-22 10:00 UTC                           |
     |    Cory -> Dan (completed)                                 |
     |    Proposed: code-review pact                             |
 ...
 [8] Meanwhile, Dan checks inbox (thread-aware)
     ...
     +----------------------------------------------------------+
     |                                                            |
     |  [1] Thread: design-pact (2 rounds)                      |
     |      From: Cory                                            |
     |      Latest: 2026-02-23 09:00 UTC                          |
     |      Round 2/refine — code-review pact                    |
...
|---|--------|------|---------|-----------------|
| 1 | Send round 1 request | pact_request | Proposing | request JSON |
| 2 | Auto-assign thread_id | pact_request (internal) | Confident | thread_id = request_id |
| 3 | Dan responds round 1 | pact_respond | - | response JSON |
| 4 | Send round 2 request | pact_request (with thread_id) | Iterating | new request JSON, same thread_id |
| 5 | Want to see conversation | Natural language | "Where did we leave off?" | - |
| 6 | Agent queries thread | pact_thread | - | All requests/responses in thread |
| 7 | Thread view displayed | Agent output | Oriented | Thread summary |
| 8 | Dan sees threaded inbox | pact_inbox | Oriented | Grouped inbox entry |
| 9 | Collaboration continues | pact_respond | - | - |

## Key Design Decisions

### Auto Thread ID (Step 2)
When no thread_id is provided, pact_request sets thread_id = request_id. This means every request is always part of a thread (even if a single-round thread). The design-pact contract already documents this convention manually; now the server enforces it.

### Thread Scan Strategy (Step 6)
pact_thread scans ALL directories (pending, completed, cancelled) and responses. A thread spans the full lifecycle. The tool does not filter by status -- it shows the complete history.

### Inbox Grouping (Step 8)
When multiple pending requests share a thread_id, pact_inbox groups them into a single entry showing the latest round. The receiver sees "Thread: design-pact (2 rounds)" instead of 2 separate inbox items. Only requests in pending/ addressed to the current user are grouped.

### Thread Summary Fields
pact_thread returns: participants (unique senders/recipients), round_count, latest_status, and chronological list of request+response pairs.

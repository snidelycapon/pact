Feature: Thread Management — Multi-Round Conversations
  ...
  Background:
    Given Cory and Dan both have configured MCP servers for the shared GARP repo
    And the skill file "skills/design-skill/SKILL.md" exists in the repo

  # --- Auto Thread ID ---

  Scenario: First request in a conversation auto-assigns thread_id
    Given Cory sends a design-skill request to Dan with no thread_id
    And the context_bundle contains skill_name "code-review", round 1, phase "propose"
    When garp_request creates the request envelope
  ...
  Scenario: Follow-up request preserves the provided thread_id
    ...
    When Cory sends a new design-skill request with thread_id "req-20260222-100000-cory-a1b2"
